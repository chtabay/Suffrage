-- LE RÉCAPITULATIF HEBDOMADAIRE — un bilan, pas un classement.
--
-- ⚠️ CE N'EST PAS UN SECOND CONCOURS. La saison mensuelle reste la seule
-- compétition ; l'hebdo ne décerne rien, ne gèle rien, et n'a pas de palmarès.
-- Il dit ce que la semaine écoulée a donné : combien de journées jouées, combien
-- de points, la meilleure. C'est un miroir, pas une échéance de plus.
--
-- ⚠️ ET IL SUIT LA RÈGLE 4 COMME LES AUTRES : seulement à qui a joué au moins
-- une journée de cette semaine-là. Un récapitulatif « vous n'avez rien joué »
-- est un rappel déguisé, et c'est exactement ce que le §7 a refusé.
--
-- ⚠️ IL EST OUVERT TROIS JOURS, PAS UN SEUL. Ne le déclencher que le lundi
-- perdrait tout joueur dont l'heure locale n'entre jamais dans le créneau ce
-- jour-là — et un abonné de Kiritimati n'y peut rien. Le registre porte la
-- SEMAINE, donc il ne part qu'une fois quoi qu'il arrive ; l'ouvrir du lundi au
-- mercredi ne fait que lui donner trois chances d'attraper son destinataire.
--
-- ⚠️ ET LA SEMAINE EST LUE SUR `cree_le`, comme la saison — pas sur le numéro de
-- journée. Les deux jeux n'ont ni la même origine ni la même charnière ;
-- l'horodatage, lui, est le même pour tout le monde. C'est aussi pourquoi
-- `scrutin_banalo_rattacher` porte l'heure RÉELLE de la réponse depuis le 28.
--
-- ⚠️ IL PASSE DERRIÈRE LA MÉDAILLE ET DEVANT LA CLÔTURE. Le lundi qui suit une
-- fin de mois, les trois genres se présentent ensemble pour un même jeu : la
-- base n'en accepte qu'un, donc la saison mène et les deux autres se replient
-- dans son texte. L'ordre `saison > hebdo > journee` est écrit dans le `choix`.
...
begin
  ...
  pays as (
    select r.user_id, r.jour, r.essais, r.cree_le, j.rang::int as rang, j.exaequo::int as exaequo
