// Emails du cercle (rendu serveur, indépendant de next-intl).
//
...
interface CircleArgs {
  ...
  leaveUrl?: string;
  /** Engagement de fréquence réellement pris par CE cercle. `null` = aucun, on n'affiche rien. */
  perDay?: number | null;
}
...
// L'engagement de fréquence n'est affiché QUE si ce cercle en a réellement pris
// un. Sinon on ne promet rien — jamais un chiffre générique à la place du cercle.
function pledge(locale: string, perDay?: number | null): string {
  if (perDay === null || perDay === undefined) return "";
  const s: Record<string, string> = {
    fr: `<br><br>Ce cercle s'engage à ne pas ouvrir plus de <b>${perDay}</b> consultation(s) par jour.`,
    en: `<br><br>This circle commits to opening no more than <b>${perDay}</b> consultation(s) per day.`,
    es: `<br><br>Este círculo se compromete a no abrir más de <b>${perDay}</b> consulta(s) por día.`,
    pcm: `<br><br>Dis circle promise say e no go open pass <b>${perDay}</b> consultation(s) per day.`,
  };
...
const CONFIRM: Record<string, (a: CircleArgs) => Parts> = {
  fr: (a) => ({
    ...
    intro: `Bonjour ${a.memberName},<br><br>Vous avez demandé à rejoindre le cercle « <b>${a.circleName}</b> » sur Placet. Cliquez ci-dessous pour confirmer — <b>tant que vous ne l'avez pas fait, vous n'y êtes pas inscrit·e</b> et votre adresse n'est enregistrée nulle part.${pledge("fr", a.perDay)}<br><br>Si vous n'êtes pas à l'origine de cette demande, ignorez simplement cet email.`,
  ...
  en: (a) => ({
    ...
    intro: `Hello ${a.memberName},<br><br>You asked to join the circle "<b>${a.circleName}</b>" on Placet. Click below to confirm — <b>until you do, you are not a member</b> and your address is stored nowhere.${pledge("en", a.perDay)}<br><br>If you didn't make this request, simply ignore this email.`,
