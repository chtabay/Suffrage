...
const LL_ES: Record<CountingMethod, string> = {
  majority: "mayoritario",
  condorcet: "Condorcet",
  mj: "juicio mayoritario",
  borda: "Borda",
  approval: "aprobación",
...
export function describeRecipe(r: Recipe, locale: string = "fr"): RecipeDescription {
  ...
  if (r.suffrage === "indirect") {
    ...
    return {
      ...
      shortName: pickLocale(locale, {
        fr: `Grands électeurs · ${ll[r.localCounting]}`,
        en: `Electoral college · ${ll[r.localCounting]}`,
        es: `Colegio electoral · ${ll[r.localCounting]}`,
      }),
      name: pickLocale(locale, {
        fr: `Grands électeurs · ${ll[r.localCounting]}${r.electorSplit === "prop" ? " (prop.)" : ""}`,
        en: `Electoral college · ${ll[r.localCounting]}${r.electorSplit === "prop" ? " (prop.)" : ""}`,
        es: `Colegio electoral · ${ll[r.localCounting]}${r.electorSplit === "prop" ? " (prop.)" : ""}`,
      }),
      how: pickLocale(locale, {
        fr: `${s.how} Décompte local choisi : « ${ll[r.localCounting]} » ; ${r.electorSplit === "wta" ? "le champion rafle tous ses grands électeurs (winner-take-all)" : "les grands électeurs sont répartis à la proportionnelle"}.`,
        en: `${s.how} Local counting method: "${ll[r.localCounting]}"; ${r.electorSplit === "wta" ? "the local champion takes all of the district's electors (winner-take-all)" : "the electors are shared proportionally"}.`,
        es: `${s.how} Método de recuento local: « ${ll[r.localCounting]} »; ${r.electorSplit === "wta" ? "el campeón local se lleva todos los grandes electores de su circunscripción (winner-take-all)" : "los grandes electores se reparten de forma proporcional"}.`,
      }),
      pros: s.pros,
      cons: s.cons,
    };
  ...
  if (r.counting === "majority" && r.rounds === 2)
    name = pickLocale(locale, {
      fr: "Majoritaire à deux tours",
      en: "Two-round runoff",
      es: "Mayoritario a dos vueltas",
    });
  else if (r.rounds === 2 && r.counting !== "majority")
    name = pickLocale(locale, {
      fr: `${base.name} à deux tours`,
      en: `${base.name} (two rounds)`,
      es: `${base.name} a dos vueltas`,
    });
  return {
    color: base.color,
    icon: base.icon,
    ...
    how:
      ...
      (r.rounds === 2 && r.counting !== "majority"
        ? pickLocale(locale, {
            fr: ` Variante à deux tours : on qualifie d'abord les meilleurs, puis ce décompte départage entre eux.`,
            en: ` Two-round variant: the best options are qualified first, then this counting method decides between them.`,
            es: ` Variante a dos vueltas: primero se clasifican los mejores y luego este recuento decide entre ellos.`,
          })
        : ""),
    pros: base.pros,
    cons: base.cons,
...
export function compute(ctx: ComputeCtx, locale: string = "fr"): ComputeResult | null {
  ...
  if (r.suffrage === "direct" && r.counting === "list") {
    ...
    return {
      ...
      tallyLabel: pickLocale(locale, {
        fr: `Répartition des ${SEATS} sièges`,
        en: `Allocation of the ${SEATS} seats`,
        es: `Reparto de los ${SEATS} escaños`,
      }),
      steps: pickLocale(locale, {
        ...
        es: [
          { n: 1, text: `Se vota por una lista entera. ${w.name} encabeza el recuento (${pct(votes[win], total)}% de los votos).` },
          { n: 2, text: `Prima de mayoría: la lista ganadora recibe automáticamente la mitad de los escaños (${bonus}).` },
          { n: 3, text: `Los ${rest} escaños restantes se reparten de forma proporcional entre las listas por encima del 5 % (prima incluida).` },
          { n: 4, text: `En total, ${w.name} obtiene una mayoría clara de ${seats[win]} escaños sobre ${SEATS}.` },
        ],
      }),
      counterfactual: pickLocale(locale, {
        fr: `La prime majoritaire transforme une courte avance en majorité confortable — c'est tout l'objectif : pouvoir gouverner.`,
        en: `The majority bonus turns a narrow lead into a comfortable majority — that's exactly the point: being able to govern.`,
        es: `La prima de mayoría convierte una ventaja escasa en una mayoría cómoda — ese es justo el objetivo: poder gobernar.`,
      }),
    };
  }
  ...
  if (r.suffrage === "direct" && r.counting === "proportional") {
    ...
    return {
      ...
      steps: pickLocale(locale, {
        ...
        es: [
          { n: 1, text: `Los ${SEATS} escaños se reparten de forma proporcional a los votos (método d'Hondt).` },
          { n: 2, text: `Las listas por debajo del 5 % no obtienen ningún escaño.` },
          { n: 3, text: `${w.name} tiene la mayor cantidad de escaños (${w.value}), pero tendrá que pactar con las demás.` },
        ],
      }),
      counterfactual: pickLocale(locale, {
        fr: `Pas de « tout au gagnant » : chaque sensibilité est représentée à hauteur de son poids réel.`,
        en: `No "winner-take-all": every current is represented in proportion to its real weight.`,
        es: `Sin « todo para el ganador »: cada sensibilidad queda representada en proporción a su peso real.`,
      }),
    };
  }
  ...
  if (r.suffrage === "indirect") {
    ...
    const allBars = bars(toVals(electors), all, (v) =>
      pickLocale(locale, {
        fr: `${v} électeurs`,
        en: `${v} electors`,
        es: `${v} electores`,
    ...
    return {
      ...
      bars: allBars,
      tallyLabel: pickLocale(locale, {
        fr: "Grands électeurs obtenus",
        en: "Electors won",
        es: "Grandes electores obtenidos",
      }),
      steps: pickLocale(locale, {
        fr: [
        ...
        es: [
          { n: 1, text: `Las papeletas se reparten en ${D} circunscripciones.` },
          { n: 2, text: `Cada circunscripción designa a su campeón mediante el recuento « ${desc.decisiveLabel} ».` },
          {
            n: 3,
            text:
              r.electorSplit === "wta"
                ? `El campeón local se lleva TODOS los grandes electores de su circunscripción (winner-take-all).`
                : `Los grandes electores de cada circunscripción se reparten de forma proporcional.`,
          },
          { n: 4, text: `En total, ${w.name} acumula la mayor cantidad de grandes electores (${w.value}) y se impone.` },
        ],
      }),
      counterfactual: upset
        ? pickLocale(locale, {
            fr: `⚠️ ${opts[popW].name} a le plus de voix au total mais perd : c'est l'effet « vote populaire vs grands électeurs ».`,
            en: `⚠️ ${opts[popW].name} has the most votes overall but loses: that's the "popular vote vs electoral college" effect.`,
            es: `⚠️ ${opts[popW].name} tiene más votos en total pero pierde: es el efecto « voto popular vs colegio electoral ».`,
          })
        : pickLocale(locale, {
            fr: `Ici le vainqueur du collège est aussi celui du vote populaire.`,
            en: `Here the winner of the college is also the winner of the popular vote.`,
            es: `Aquí el ganador del colegio electoral es también el del voto popular.`,
          }),
    };
  }
  ...
  if (r.rounds === 2) {
    ...
    if (lead.value / total > 0.5) {
      return {
        ...
        bars: fpBars,
        tallyLabel: pickLocale(locale, {
          fr: "Premier tour — voix",
          en: "First round — votes",
          es: "Primera vuelta — votos",
        }),
        steps: pickLocale(locale, {
          fr: [
          ...
          es: [
            { n: 1, text: "Se cuentan las primeras preferencias." },
            { n: 2, text: `${lead.name} supera el 50 % (${pct(lead.value, total)}%) ya en la primera vuelta.` },
            { n: 3, text: "No hace falta una segunda vuelta." },
          ],
        }),
        counterfactual: pickLocale(locale, {
          fr: `${lead.name} est élu dès le 1er tour à la majorité absolue.`,
          en: `${lead.name} is elected in the first round with an absolute majority.`,
          es: `${lead.name} es elegido en la primera vuelta por mayoría absoluta.`,
        }),
      };
    }
    ...
    steps.push({
      ...
      text: pickLocale(locale, {
        fr: `1er tour : personne n'a la majorité absolue (${fpBars[0].name} mène avec ${pct(fpBars[0].value, total)}%).`,
        en: `First round: nobody has an absolute majority (${fpBars[0].name} leads with ${pct(fpBars[0].value, total)}%).`,
        es: `1.ª vuelta: nadie tiene la mayoría absoluta (${fpBars[0].name} encabeza con ${pct(fpBars[0].value, total)}%).`,
      }),
    });
    steps.push({
      n: sN++,
      text: pickLocale(locale, {
        fr: `Se qualifient ${r.qualif === "top2" ? "les 2 premiers" : "les candidats au-dessus de 10 %"} : ${finalists.map((i) => opts[i].name).join(", ")}.`,
        en: `Qualifying: ${r.qualif === "top2" ? "the top 2" : "the candidates above 10%"}: ${finalists.map((i) => opts[i].name).join(", ")}.`,
        es: `Se clasifican ${r.qualif === "top2" ? "los 2 primeros" : "los candidatos por encima del 10 %"}: ${finalists.map((i) => opts[i].name).join(", ")}.`,
      }),
    });
  }
  ...
  const tallyLabelMap: Record<string, string> = pickLocale(locale, {
    ...
    es: {
      majority: "Votos",
      condorcet: "Duelos ganados (Copeland)",
      mj: "Mención mediana",
      approval: "Aprobaciones",
      borda: "Puntos de Borda",
    },
  });
  const tallyLabel =
    (tallyLabelMap[method] || pickLocale(locale, { fr: "Décompte", en: "Count", es: "Recuento" })) +
    (round1
  ...
  if (method === "condorcet" && t.condorcetNull && !r.random) {
    ...
    return {
      ...
      noWinnerLabel: pickLocale(locale, {
        fr: "Paradoxe de Condorcet",
        en: "Condorcet paradox",
        es: "Paradoja de Condorcet",
      }),
      bars: resBars,
      tallyLabel,
      steps: [
        ...steps,
        {
          n: sN++,
          text: pickLocale(locale, {
            fr: `Les duels tournent en rond : aucune option ne bat toutes les autres. Cercle bloqué : ${smith.map((i) => opts[i].name).join(", ")}.`,
            en: `The duels go in circles: no option beats all the others. Deadlocked cycle: ${smith.map((i) => opts[i].name).join(", ")}.`,
            es: `Los duelos giran en círculo: ninguna opción gana a todas las demás. Ciclo bloqueado: ${smith.map((i) => opts[i].name).join(", ")}.`,
          }),
        },
        {
          n: sN++,
          text: pickLocale(locale, {
            fr: "Activez « Tirage si blocage » pour départager au sort.",
            en: `Enable "Random tie-break" to settle it by lot.`,
            es: "Active « Sorteo en caso de empate » para decidir por sorteo.",
          }),
        },
      ],
      counterfactual: pickLocale(locale, {
        fr: "Ce blocage rare illustre pourquoi des variantes existent (randomisée, Schulze…).",
        en: "This rare deadlock shows why variants exist (randomized, Schulze…).",
        es: "Este bloqueo poco frecuente muestra por qué existen variantes (aleatoria, Schulze…).",
      }),
    };
  }
  ...
  if (!round1) {
    const intro: Record<string, string> = pickLocale(locale, {
      fr: {
      ...
      es: {
        majority: "Cada papeleta cuenta solo para su única opción preferida.",
        condorcet: "Se simula cada duelo cara a cara a partir de las clasificaciones.",
        mj: "Cada votante asigna una mención; se toma la mención mediana de cada opción.",
        borda: `En cada papeleta: 1.º = ${n - 1} pts, 2.º = ${n - 2} pts… último = 0.`,
        approval: "Cada cual marca todas las opciones que le convienen.",
      },
    });
    ...
    steps.push({
      n: sN++,
      text: pickLocale(locale, {
        fr: `2nd tour : on applique le décompte « ${desc.decisiveLabel} » entre les seuls qualifiés.`,
        en: `Second round: we apply the "${desc.decisiveLabel}" count among the qualified options only.`,
        es: `2.ª vuelta: se aplica el recuento « ${desc.decisiveLabel} » solo entre los clasificados.`,
      }),
    });
  }

  const fin: Record<string, string> = pickLocale(locale, {
    fr: {
    ...
    es: {
      majority: round1 ? `${w.name} se impone en la segunda vuelta.` : `${w.name} reúne la mayor cantidad de votos.`,
      condorcet: t.condorcetNull
        ? `🎲 Ciclo bloqueado → sorteo: ${w.name} queda designado.`
        : `${w.name} gana a todas las demás opciones cara a cara: ganador de Condorcet.`,
      mj: `${w.name} tiene la mejor mención mediana: « ${GR[t.vals[t.winner]]} ».`,
      borda: `${w.name} suma la mayor cantidad de puntos (${t.vals[t.winner]}).`,
      approval: `${w.name} es aprobado por la mayor cantidad de gente (${t.vals[t.winner]} de ${total}).`,
    },
  });
  steps.push({ n: sN++, text: fin[method] });

  const cf: Record<string, string> = pickLocale(locale, {
    fr: {
      majority: round1
    ...
    es: {
      majority: round1
        ? `La segunda vuelta garantiza una mayoría absoluta — pero el 3.º, a veces mejor consenso, queda eliminado de entrada.`
        : t.vals[t.winner] / total < 0.5
          ? `${w.name} gana sin mayoría absoluta: una sola vuelta basta, pero puede dividir.`
          : `${w.name} tiene la mayoría absoluta.`,
      condorcet: `El ganador de Condorcet vencería a cada adversario en duelo — no necesariamente quien tiene más primeros puestos.`,
      mj: `Se premia el respaldo amplio en lugar de la intensidad de una pequeña minoría.`,
      borda: `Borda favorece las opciones « segundas en todo » frente a los favoritos que dividen.`,
      approval: `Se gana agradando al mayor número, no siendo el preferido de un solo bando.`,
    },
  });
