  ...
  const c = pickLocale<Content>(locale, {
    ...
    en: {
      ...
      sections: [
        ...
        {
          h: "Retention",
          p: "Polls and ballots are kept until you delete them or ask us to. Game rooms are the exception: they are deleted automatically 7 days after the game's last activity, along with the names, words and scores they hold. Daily games have no room: a player's answer or the outcome of their game (number of guesses, duration), the day number, a random identifier drawn by their browser and, if there is one, the name left on that day's board are kept for 30 days, then deleted. That identifier only serves to recognise your games from one day to the next so we can give you back your result and your standing; it is SPECIFIC TO EACH GAME, so it does not link your games across games, and it is erased as soon as you attach those games to an account. A name left without an account is for one day only: there is then no profile and no permanent nickname, and it is not copied into the account summary. If you have a Placet account, however, it is that account's name that identifies you everywhere — day board, group, leaderboards — and it is kept until you change it, remove it, or delete the account. You can remove it at any time from “Results and rankings”: it then disappears from the rankings, from past podiums and from the daily boards, and the names already left under that nickname are deleted — your results themselves stay. A table, and the names left in it, are deleted the same way, once none of its members has played for 30 days. If you choose to attach your results to an account — it is optional, and the game works without one — a summary (day number, score, language) is then kept on that account until you delete it: that is what lets a streak outlast the thirty days. Neither your answers nor your words are copied into that summary.",
        },
        {
          h: "Your rights",
